...
# Função para carregar os dados de 2025
@st.cache_data
def load_2025_data():
    return pd.read_csv("dados_2025.csv")
...
if "NOME MUNICÍPIO" in df_ibge.columns and "MUNICÍPIO" in df_2025.columns:
    df_2025.rename(columns={"MUNICÍPIO": "NOME MUNICÍPIO"}, inplace=True)
...
if "UF" in df_2025.columns:
    df_2025["UF"] = df_2025["UF"].str.upper()

# Filtrar os municípios atendidos no df_2025
df_ibge_filtrado = df_ibge[df_ibge["NOME MUNICÍPIO"].isin(df_2025["NOME MUNICÍPIO"].unique())]

# Criar abas
abas_graficos20251, aba_tabelas20251 = st.tabs(["Gráficos - 2025/1", "Tabelas - 2025/1"])

with abas_graficos20251:
    st.title("Distribuição de Matrículas por Região, Estado e Município - 2025")
